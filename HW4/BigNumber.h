...
BigNumber::BigNumber(int sgn, int length, string s){
    sign = sgn;
    num_of_bits = length;
    data.assign(MAX_Bit, 0);
    for(int i = 0; i < length; i++) {
        if(s[i] >= 65) data[length - 1 - i] = s[i] - 65 + 10;
        else data[length - 1 - i] = s[i] - 48;
    }
}
...
void BigNumber::Print_fixed(int size){
    if(sign == -1) std::cout << "-";
    if(size < num_of_bits) Print();
    else{
        for(int i = 0; i < size; i++) {
            if(i != 0 && i % 8 == 0) std::cout << std::dec << " ";
            std::cout << std:: hex << data[size - 1 - i];
        }
        std::cout << std::dec;
    }
}
